...
class BambuPrinter:
    ...
    def start_session(self):
        ...
        # if self.config.ip is None or self.config.access_code is None or self.config.serial_number is None:
        if self.config.ip is None or self.config.auth_token is None or self.config.serial_number is None:
        ...
        self.client.username_pw_set(self.config.mqtt_username_cloud, password=self.config.auth_token)
        # self.client.user_data_set(self.config.mqtt_client_id)

        try:
            self.client.connect(self.config.cloud_server, self.config.mqtt_port, 60)
            # self.client.connect(self.config.ip, self.config.mqtt_port, 60)
        except Exception as e:
    ...
    def get_sdcard_contents(self):
        ...
        The return value of this method is very useful for binding to things like a clientside `TreeView`
        """
        ftps = IoTFTPSClient(self._config.ip, 990, self._config.mqtt_username_local, self._config.access_code, ssl_implicit=True)
